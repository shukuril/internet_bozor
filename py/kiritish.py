...
def update_google_sheet(data):
    ...
    try:
        # Декодируем учетные данные Google API из base64
        ...
        # Открываем таблицу
        sheet = client.open_by_key(SPREADSHEET_ID).worksheet('form data')
...
async def process_photo(message: types.Message, state: FSMContext):
    ...
    photos_folder = r'/tmp/photos'  # Используем временную папку /tmp для Railway
